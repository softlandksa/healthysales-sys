...
interface CustomerExcelActionsProps {
  canImport: boolean;
}

export function CustomerExcelActions({ canImport }: CustomerExcelActionsProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append("file", file);
      const res  = await fetch("/api/customers/import?action=preview", { method: "POST", body: fd });
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res  = await fetch("/api/customers/import?action=import", { method: "POST", body: fd });
      const data = await res.json() as { saved?: number; error?: string };
      if (!res.ok) throw new Error(data.error ?? "حدث خطأ أثناء الاستيراد");
      toast.success(`تم استيراد ${data.saved} عميل بنجاح`);
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              ارفع ملف Excel يحتوي على بيانات العملاء. حمّل القالب أولاً للتأكد من تطابق الأعمدة.
            </DialogDescription>
          </DialogHeader>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-surface-1 sticky top-0">
                      <tr>
                        ...
                        <th className="text-right px-3 py-2 font-medium text-text-secondary">الاسم</th>
                        <th className="text-right px-3 py-2 font-medium text-text-secondary">الهاتف</th>
                        <th className="text-right px-3 py-2 font-medium text-text-secondary">الرصيد الافتتاحي</th>
                        <th className="text-right px-3 py-2 font-medium text-text-secondary">حد الائتمان</th>
                    ...
                    <tbody className="divide-y divide-border">
                      {preview.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 num">{row.phone || "—"}</td>
                          <td className="px-3 py-2 num">{row.openingBalance || "0"}</td>
                          <td className="px-3 py-2 num">{row.creditLimit || "—"}</td>
